...
- [Kursintroduktion (YouTube, 4:28)](https://youtu.be/J2bpA1XECBI)
- [Kursverktyg och examination (YouTube, 5:37)](https://youtu.be/qrgjCchBTWs)
...
- [Examination och betyg (kurshemsida)](https://kurser.math.su.se/pluginfile.php/140337/mod_resource/content/1/betygskriterier.pdf)
...
- [The Basics](https://rstudio.cloud/learn/primers/1) (Vizualisation och Programming)
...
```r
download.file("https://github.com/MT3003-ST22/data/raw/main/systembolaget_2019-10-30.csv",
...
```r
download.file("https://github.com/MT3003-ST22/data/raw/main/gapminder_2021-06-14.csv",
...
- Gör en tabell över medelinkomsten per capita för varje kontinent 2020 baserat på Gapminder data från förra veckans övningar. Tänk på att även `income` är räknat i per capita, du behöver alltså bestämma kontinenternas totala inkomst och dela på den totala populationen.
...
Hos Havs och Vattenmyndigheten kan du hitta [statistik över vattenprover](https://havbipub.havochvatten.se/analytics/saw.dll?PortalPages&PortalPath=%2Fshared%2FHemsidan%2FBadplats%2F_portal%2FBadplats) tagna vid badplatser runt om i landet, ett utdrag i Excel-format kan laddas ned med 

```r
download.file("https://github.com/MT3003-ST22/data/raw/main/Provresultat.xlsx",
...
Ovanstående utdrag hämtades i maj då det fortfarande var ganska kallt i vattnet. Vill du hämta aktuella data på samma format väljer du *Exportera > Formaterad > Excel* längst ned till vänster på sidan.
...
```r
download.file("https://github.com/MT3003-ST22/data/raw/main/Provresultat.csv",
...
```r
download.file("https://github.com/MT3003-ST22/data/raw/main/cell_phones_total_2021-06-27.csv",
...
```r
download.file("https://github.com/MT3003-ST22/data/raw/main/LanSweref99TM.zip",
              "LanSweref99TM.zip")
...
Amineh Kakabavehs roll som politisk vilde i riksdagen har väckt en del uppmärksamhet på sistone. Men hur ofta röstar egentligen moderpartit Vs ledamöter samma som henne?

Med 

```r
votering_V <- read_csv("http://data.riksdagen.se/voteringlista/?bet=&punkt=&parti=V&valkrets=&rost=&iid=&sz=10000&utformat=csv&gruppering=")
...
Gör motsvarande jämförelse mellan V-ledamöter och politiske vilden Roger Richthoff (tidigare SD). Eftersom Roger blev vilde först under 2022 har han inte deltagit som vilde i alla voteringar i `votering_V`, ta bort de där han inte voterat som vilde.
...
Koden bakom en figur skapad med `ggplot` kan bli många rader om man vill finjustera detaljer. Skall man upprepa en figur för olika parametervärden är det därför praktiskt att skriva en funktion. Använd tabellen över 20000 försäljningar i innerstaden som laddas ned med

```r
download.file("https://github.com/MT3003-ST22/data/raw/main/booli_alla_2022-05-24.csv",
              "data/booli_alla_2022-05-24.csv")
booli_data <- read_csv("data/booli_alla_2022-05-24.csv")
```
...
På [https://sv.wikipedia.org/wiki/Medaljf%C3%B6rdelning_vid_olympiska_sommarspelen_2020](https://sv.wikipedia.org/wiki/Medaljf%C3%B6rdelning_vid_olympiska_sommarspelen_2020) hittar du  medaljfördelningen för OS i Tokyo. Tabellen kan hämtas med
...
```
som returnerar medaljtabellen för OS-år `year` som nedan (glöm inte ta bort Total-raden). Tyvärr är inte alla kolumnnamn helt konsistenta, men du behöver bara kolumnerna `Land`, `Guld`, `Silver` och `Brons`.
...
I en [tidigare övning](https://mt3003-st22.github.io/schedule.html#v28:_Omforma_och_sammanfl%C3%A4ta_tabeller) omformade vi en csv-fil från gapminder till ett mer användbart format. Nedanstående kod hämtar fler filer och placerar dem i `data/gapminder_raw` 
...
##  1 1     NA    ""    Malmö FF          6     6     0     0     17-3  18   
##  2 2     NA    ""    BK Häcken         6     5     0     1     17-7  15   
##  3 3     NA    ""    IF Elfsborg       6     4     1     1     14-4  13   
##  4 4     NA    ""    Kalmar FF         6     3     2     1     9-4   11   
##  5 5     NA    ""    IFK Norrköping    6     3     2     1     9-6   11   
##  6 6     NA    ""    IF Brommapojkarna 6     3     0     3     7-10  9    
##  7 7     NA    ""    Mjällby AIF       6     2     2     2     5-5   8    
##  8 8     NA    ""    Djurgården        6     2     1     3     6-9   7    
##  9 9     NA    ""    IFK Värnamo       6     2     0     4     4-7   6    
## 10 10    NA    ""    Hammarby          6     2     0     4     8-12  6    
## # ℹ 23 more rows
```
där SelectorGadget har använts för att hitta väljaren `.col-md-6`. Notera att kolumnernas namn inte är fullständiga; tabellen är ju inte avsedd för skrapning och resultatet behöver därför ofta justeras. Vid webbskrapning är det viktigt att respektera att syftet i allmänhet inte är att dela data och därför inte belasta servrar i onödan. Upphovsrätten till data kan även vara skyddat genom [katalogskyddet](https://lagen.nu/1960:729#P49).
